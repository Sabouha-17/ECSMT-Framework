...
print("=================Graph Regularization Defense=================")
...
# For evaluation: generate poisoned and clean test sets
if args.attack_method == "subgraph":
    _, _, backdoor_test_dataset, _ = inject_sub_trigger(
        args,
        copy.deepcopy(test_dataset),
...
log_name = (
    args.un_log_dir
    + "/{}_{}_{}_ratio{}_triggerratio{}_reglr{}_{}.csv".format(
        args.dataset,
        args.model,
        args.attack_method,
        args.injection_ratio,
...
lambda_reg = 1e-3  # strength of graph regularization
...
for epoch in range(1, args.un_epoch + 1):
    ...
    for data in train_loader:
        ...
        # Node embeddings from final conv layer (used already in unlearning / Grad-CAM)
        node_emb = model.final_conv_acts  # shape: [num_nodes, hidden_dim]

        # Graph smoothness: neighboring nodes should have similar embeddings
        row, col = data.edge_index
        edge_diff = node_emb[row] - node_emb[col]
        smooth_reg = (edge_diff.pow(2).sum(1)).mean()

        loss = ce_loss + lambda_reg * smooth_reg
