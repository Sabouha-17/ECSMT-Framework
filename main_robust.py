...
print("=================Robust Training Defense=================")

# Load full dataset again for defense phase
dataset, num_classes = load_data(args.dataset)
...
# Build backdoor and clean test sets for evaluation only
if args.attack_method == "subgraph":
    _, _, backdoor_test_dataset, _ = inject_sub_trigger(
        args,
        copy.deepcopy(test_dataset),
...
# New optimizer for robust fine-tuning
...
log_name = (
    args.un_log_dir
    + "/{}_{}_{}_ratio{}_triggerratio{}_robustlr{}_{}.csv".format(
        args.dataset,
        args.model,
        args.attack_method,
        args.injection_ratio,
...
for epoch in range(1, args.un_epoch + 1):
    ...
    for data in train_loader:
        ...
        # Clean forward
        output_clean = model(data.x, data.edge_index, data.batch)
        loss_clean = criterion(output_clean, data.y)

        # Robust / adversarial-style training:
        # Add small Gaussian noise to node features
        noisy_x = data.x + 0.1 * torch.randn_like(data.x)
        output_noisy = model(noisy_x, data.edge_index, data.batch)
        loss_noisy = criterion(output_noisy, data.y)

        # Total robust loss
        loss = loss_clean + 0.5 * loss_noisy  # 0.5 = robustness weight
